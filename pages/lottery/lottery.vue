			...
			<view class="buylist">
				...
				<view class="blist">
					<view class="li" 
						...
						<view class="t2">{{roomtype=="3"?"1:":""}}{{item.odds}}</view>
					</view>
...
<script>
	...
	export default {
		...
		async onLoad(option) {
			console.log(option.roomType,'111111111111')
		...
		methods: {
			...
			buy(){
				...
				if(roomtype == 0){
					...
					if(this.buynumber >= config.thresholdChuMin){
						...
						this.closemodal()
						this.showmoadl('下单数量不能低于￥'+config.thresholdChuMin)
					}
